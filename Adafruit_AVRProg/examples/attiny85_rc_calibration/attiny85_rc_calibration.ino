...
void loop(void) {
  ...
  mySerial.print(F("Erasing chip again..."));
  avrprog.eraseChip();
  mySerial.println(F("Done!"));
  ...
  if (!avrprog.verifyFuses(targetimage->image_normfuses,
    ...
    mySerial.println("Fuses verified correctly!");
  }
  ...
  if (osscal_value == 0xFF) {
    ...
    mySerial.println("RC calibrated correctly!");
  }

  mySerial.print(F("Erasing chip one more time..."));
  avrprog.eraseChip();
  mySerial.println(F("Done!"));
