...
void setup() {
  ...
  while (!avrprog.targetPower(true)) {
    ...
    mySerial.println("Failed to connect to target, power cycling");
    power_cycle();
    delay(500);
  }
  mySerial.println("UPDI Powered & Responded!");
  ...
  while (1) {
    ...
    if (avrprog.g_updi.initialized && !avrprog.g_updi.unlocked) {
      mySerial.println("OK so the chip is locked!");
      avrprog.UPDIunlock();
    } else if ((signature != 0) && signature != 0xFFFF) {
      break;
    } else {
      mySerial.println(F("No target attached?"));
    }
    power_cycle();
    delay(500);
  }
  ...
  for (uint8_t f=0; f<10; f++) {
    //// mySerial.printf("Fuse #%d = 0x%02X", f, fuses[f]);
    uint8_t newfuse = targetimage->image_normfuses[f];
    if (newfuse != fuses[f]) {
      //// mySerial.printf(" -> 0x%02X\n", newfuse);
      wrotefuse = true;
